...
Presentcardtext = "Please Enter PIN"
    ...
    if len(entered_pin) == 6 or key == "#":
        try:
            ...
            if result is not None:
                ...
                lcd.lcd_clear()
                lcd.lcd_display_string(dspText,1)
                correct_pin_entered(dspText)
                db.commit()
            else:
                db.commit()
...
def initialize_lcd():
    lcd.lcd_display_string("Asya Software",1)
